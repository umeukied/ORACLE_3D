...
!.....CORRECTION ROBUSTE : si DELN < 0, la normale pointe vers l interieur
!.....On inverse pour garantir des flux corrects quelle que soit l orientation du GRD
    IF(DELN < 0.0_RK) THEN
       ARX  = -ARX
       ARY  = -ARY
       ARZ  = -ARZ
  ...
  SUBROUTINE INIT_GEOM_T(X,Y,Z,XC,YC,ZC, &
    ...
    ARE2_T  =MAX(ARX_T**2+ARY_T**2+ARZ_T**2,0._rk)
    ARE_T   =SQRT(ARE2_T)
...
!.....CORRECTION ROBUSTE : inverser les aires internes si DELN < 0
    WHERE(DELN_T < 0.0_RK)
       ARX_T   = -ARX_T
       ARY_T   = -ARY_T
       ARZ_T   = -ARZ_T
